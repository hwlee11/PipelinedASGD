...
class ResNetBasicblock(nn.Module):
  ...
  """
  def __init__(self, inplanes, planes, stride=1, downsample=None,group=2):
    super(ResNetBasicblock, self).__init__()

    self.conv_a = nn.Conv2d(inplanes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
    #self.bn_a = nn.BatchNorm2d(planes,affine=False)
    self.bn_a = nn.GroupNorm(group,planes)

    self.conv_b = nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
    #self.bn_b = nn.BatchNorm2d(planes,affine=False)
    self.bn_b = nn.GroupNorm(group,planes)
...
class CifarResNet(object):
  ...
  """
  def __init__(self, block, depth, num_classes,group):
    """ Constructor
    Args:
    ...
    self.bn_1 = nn.GroupNorm(group,16)
    ...
    for i, planes in enumerate(list_planes):
      ...
      #if planes == 
      #print('i,planes',i,planes)
      block_group = int( planes / 2)
      #print(group)
      # add block into self.layers
      self.layers.append(block(self.inplanes, planes, stride, downsample, block_group))
...
#def resnet_ddg(depth, num_classes=10, num_splits=2, group=2):
def resnet_gn(depth, num_classes=10, num_splits=2, group=2):
  """
  construct resnet network.
  """
  model = CifarResNet(ResNetBasicblock, depth, num_classes, group)
  ...
  for splits_id in range(num_splits):
    ...
    print('splits_id',splits_id)
    print(len_layers, split_depth)
    print('model split',left_idx,right_idx)
    net = CifarResNetDDG(model, model.layers[left_idx:right_idx], splits_id, num_splits, num_splits-1-splits_id)
    nets.append(net) 
  '''
  net = CifarResNetDDG(model,model.layers[0:16],0,num_splits,4)
  nets.append(net)
  net = CifarResNetDDG(model,model.layers[16:30],1,num_splits,4)
  nets.append(net)
  net = CifarResNetDDG(model,model.layers[30:44],2,num_splits,4)
  nets.append(net)
  net = CifarResNetDDG(model,model.layers[44:58],3,num_splits,4)
  nets.append(net)
  '''
...
def resnet_ddg_gn_110(depth, num_classes=10, num_splits=2):

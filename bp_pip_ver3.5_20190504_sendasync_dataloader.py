...
def main(args):
    ...
    # restnet model list
    nets = models.__dict__[args.arch](args.depth,num_classes=num_classes,num_splits=split,group=16)##################
    ...
    for rank in range(split):
            if rank == 0:
                    ...
                    p.start()
                    Processes.append(p)
                    p = Process(target=input_layer,args=( sh_list,sh_test,sh_c_list,shm_list , train_loader,test_loader,nets[rank],rank,split,batch_size,batch_num,test_batch_num,epoch_num,lamda,lr,cv))
                    p.start()
                    Processes.append(p)
                    ...
                    p.start()
                    Processes.append(p)
                    p = Process(target=hidden_layer,args=( sh_list,sh_test,sh_c_list,nets[rank],rank,split,batch_num,test_batch_num,epoch_num,lamda,lr,cv))
                    p.start()
                    Processes.append(p)

            elif rank == (split - 1) :
                    p = Process(target=output_layer,args=( sh_list,sh_test,sh_c_list,shm_list ,train_loader,test_loader,nets[rank],loss_function,rank,split,batch_size,batch_num,test_batch_num,test_num, epoch_num,lamda,lr,cv))
                    p.start()
                    Processes.append(p)
...
def input_layer( sh_list ,sh_test, sh_c_list, shm_list ,train_loader,test_loader ,model,rank,split,batch_size,batch_num,test_batch_num,epoch_num,lamda,lr,cv):
...
def hidden_layer( sh_list,sh_test,sh_c_list,model,rank,split,batch_num,test_batch_num,epoch_num,lamda,lr,cv):
...
def output_layer( sh_list ,sh_test, sh_c_list,shm_list , train_loader ,test_loader ,model,loss_function,rank,split,batch_size,batch_num,test_batch_num,test_num,epoch_num,lamda,lr,cv):
        ...
        for epoch in range(epoch_num):
                ...
                #for i in range(test_batch_num) :
                        ...
                        #i += 1
                print('epoch=',epoch,'tot_time =',time_tot,'accuracy =', (100*correct/test_num),'test_loss',dev_loss_tot/test_batch_num)
